...
**profile**: a metapackage `op-<device>-profile-<name>` whose `Depends:`
...
| Repo | Role |
|---|---|
| `packages` (this repo) | `debian/` tree per package: all profile metapackages, `op-archive-keyring`, `op-*-base` (as rootfs content migrates), small tools |
...
Naming: `op-` prefix everywhere; `op-<device>-profile-<name>` for
profiles, `op-app-<name>` for apps, `arch: all` unless it ships binaries.
...
`op-tc8-profile-<name>` from the archive, stamps `TC8_PROFILE` into
...
Say `op-tc8-profile-signage`:

1. **Metapackage** — in this repo:
   ```
   mkdir -p op-tc8-profile-signage/debian/source
   ```
   `debian/control`:
   ```
   Source: op-tc8-profile-signage
   ...
   Package: op-tc8-profile-signage
   Architecture: all
   Depends: ${misc:Depends}, op-app-yourthing, some-debian-pkg
...
3. **Test on a device without any publishing** (fastest loop):
   ```
   cd op-tc8-profile-signage && dpkg-buildpackage -us -uc -b
   scp ../op-tc8-profile-signage_*.deb root@<panel>:/tmp/
...
4. **Publish**: push to this repo (CI builds + dispatches), or the manual
   path above. Verify: `apt update && apt-cache policy op-tc8-profile-signage`
   on any device.
5. **Image variant**:
   ```
...
- **Key hygiene**: never commit the private key; the public keyring is the
  `op-archive-keyring` package + baked into the bases.
